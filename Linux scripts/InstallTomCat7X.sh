...
#[ -z $6 ] && echo $DATE_WITH_TIME "TomCat service  missing missing" | tee -a /opt/deploy.log  | exit 1 ## tomcat directory
...
if [ -d "$1" ] && [ -d "$5" ]; then
        ...
        ## Insert agent path into setenv.sh 
			echo $DATE_WITH_TIME "Inserting agent path to setenv.sh" | tee -a /opt/deploy.log
		...
		## check if the agent path has been inserted , exit if it does not find it
			...
			if [[ ! -x "$5"bin/setenv.sh ]]; then
				...
				if [ $? -eq 1 ]; then	
					echo $DATE_WITH_TIME "Failed to make setenv.sh executable" | tee -a /opt/deploy.log
					
					### ROLLBACK if the unable to make setenv.sh executable and remove agent pathing
					echo $DATE_WITH_TIME "Rolling Back" | tee -a /opt/deploy.log
