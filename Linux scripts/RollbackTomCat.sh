...
[ -z $1 ] && echo $DATE_WITH_TIME "TomCat directory missing missing" | tee -a /opt/Rollback.log | exit 1 ## tomcat directory
[ -z $2 ] && echo $DATE_WITH_TIME "Is version 7 boolean has not been inputted" | tee -a /opt/Rollback.log | exit 1 ## tomcat directory

echo $DATE_WITH_TIME "Checking which version of tomcat has been inputted in" | tee -a /opt/Rollback.log 

## checks if rolling back for version 7 or not. "true" if version 7 
if [ "$2" = "true" ]; then

	echo $DATE_WITH_TIME "Rollback for Tomcat 7" | tee -a /opt/Rollback.log 
	echo $DATE_WITH_TIME "Removing agent path from setenv.sh" | tee -a /opt/Rollback.log 
	...
	echo $DATE_WITH_TIME "Rollback for Tomcat 6 and below" | tee -a /opt/Rollback.log 
	echo $DATE_WITH_TIME "Removing agent path from catalina.sh" | tee -a /opt/Rollback.log 
